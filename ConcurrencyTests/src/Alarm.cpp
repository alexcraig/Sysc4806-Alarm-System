...
// {{{RME transition ':TOP:Armed:J4D6D43EA0218:TriggerAlarm'
INLINE_METHODS void Alarm_Actor::transition4_TriggerAlarm( const void * rtdata, RTProtocol * rtport )
{
	// {{{USR
	PeripheralIdentifier ident;
	ident.peripheralType = TYPE_ALARM;
...
INLINE_CHAINS void Alarm_Actor::chain4_TriggerAlarm( void )
{
	...
	exitState( rtg_parent_state );
	rtgTransitionBegin();
	transition4_TriggerAlarm( msg->data, msg->sap() );
	rtgTransitionEnd();
	enterState( 5 );
}
