...
int CellHandler_Actor::_followInV( RTBindingEnd & rtg_end, int rtg_portId, int rtg_repIndex )
{
	...
	case 2:
		...
		{
			// sensorR1/testPort
			return sensorR1._followIn( rtg_end, 1, rtg_repIndex );
		}
		break;
	case 3:
		// failAlarms
		...
		{
			// alarmR1/testPort
			return alarmR1._followIn( rtg_end, 1, rtg_repIndex );
		}
		break;
	default:
		break;
...
INLINE_METHODS void CellHandler_Actor::transition4_LaunchSelfTest( const void * rtdata, RTProtocol * rtport )
{
	// {{{USR
	int i;
...
INLINE_METHODS void CellHandler_Actor::transition5_SelfTestComplete( const void * rtdata, Timing::Base * rtport )
{
	...
	testSystem.selfTestComplete().send();
	// }}}USR
}
// }}}RME
...
INLINE_METHODS void CellHandler_Actor::transition7_Disarm( const void * rtdata, RTProtocol * rtport )
{
	// {{{USR
...
INLINE_METHODS void CellHandler_Actor::transition10_SoundAlarms( const void * rtdata, RTProtocol * rtport )
{
	// {{{USR
...
INLINE_METHODS void CellHandler_Actor::transition12_testAlarmActive( const PeripheralIdentifier * rtdata, TestProtocol::Base * rtport )
{
	// {{{USR
	int portIndex = msg->sapIndex0();
	String identifier = char(cellIndex) + "test";
...
INLINE_METHODS void CellHandler_Actor::transition13_testAlarmArmed( const PeripheralIdentifier * rtdata, TestProtocol::Base * rtport )
{
	// {{{USR
	int portIndex = msg->sapIndex0();
	String identifier = char(cellIndex) + "test";
...
INLINE_METHODS void CellHandler_Actor::transition14_testAlarmDisarmed( const PeripheralIdentifier * rtdata, TestProtocol::Base * rtport )
{
	// {{{USR
	int portIndex = msg->sapIndex0();
	String identifier = char(cellIndex) + "test";
	...
	ident.cellIndex = this->cellIndex;
	ident.peripheralIndex = portIndex;
	testSystem.disarmed(ident).send();
	// }}}USR
}
// }}}RME

// {{{RME transition ':TOP:Idle:J4D7FD0790109:testSensorDisarmed'
INLINE_METHODS void CellHandler_Actor::transition15_testSensorDisarmed( const PeripheralIdentifier * rtdata, TestProtocol::Base * rtport )
{
	// {{{USR
	int portIndex = msg->sapIndex0();
	String identifier = char(cellIndex) + "test";
...
{
	// transition ':TOP:Idle:J4D767B300193:Arm'
	rtgChainBegin( 2, "Arm" );
...
{
	// transition ':TOP:Idle:J4D767B420116:Disarm'
	rtgChainBegin( 2, "Disarm" );
	exitState( rtg_parent_state );
	rtgTransitionBegin();
	transition7_Disarm( msg->data, msg->sap() );
	rtgTransitionEnd();
	enterState( 2 );
}
...
INLINE_CHAINS void CellHandler_Actor::chain4_LaunchSelfTest( void )
{
	...
	exitState( rtg_parent_state );
	rtgTransitionBegin();
	transition4_LaunchSelfTest( msg->data, msg->sap() );
	rtgTransitionEnd();
	enterState( 2 );
}
...
{
	// transition ':TOP:Idle:J4D767B7D0358:SoundAlarms'
	rtgChainBegin( 2, "SoundAlarms" );
	exitState( rtg_parent_state );
	rtgTransitionBegin();
	transition10_SoundAlarms( msg->data, msg->sap() );
	rtgTransitionEnd();
	enterState( 2 );
}
...
{
	// transition ':TOP:Idle:J4D767B750154:BreakInTriggered'
...
{
	// transition ':TOP:Idle:J4D767B6C0125:SelfTestComplete'
...
INLINE_CHAINS void CellHandler_Actor::chain15_testSensorDisarmed( void )
{
	// transition ':TOP:Idle:J4D7FD0790109:testSensorDisarmed'
	rtgChainBegin( 2, "testSensorDisarmed" );
	exitState( rtg_parent_state );
	rtgTransitionBegin();
	transition15_testSensorDisarmed( (const PeripheralIdentifier *)msg->data, (TestProtocol::Base *)msg->sap() );
	rtgTransitionEnd();
	enterState( 2 );
}
...
{
	// transition ':TOP:Idle:J4D767BA6025D:testSensorArmed'
...
INLINE_CHAINS void CellHandler_Actor::chain12_testAlarmActive( void )
{
	// transition ':TOP:Idle:J4D76839C0194:testAlarmActive'
	rtgChainBegin( 2, "testAlarmActive" );
	exitState( rtg_parent_state );
	rtgTransitionBegin();
	transition12_testAlarmActive( (const PeripheralIdentifier *)msg->data, (TestProtocol::Base *)msg->sap() );
	rtgTransitionEnd();
	enterState( 2 );
}

INLINE_CHAINS void CellHandler_Actor::chain14_testAlarmDisarmed( void )
{
	// transition ':TOP:Idle:J4D7FD059008C:testAlarmDisarmed'
	rtgChainBegin( 2, "testAlarmDisarmed" );
	exitState( rtg_parent_state );
	rtgTransitionBegin();
	transition14_testAlarmDisarmed( (const PeripheralIdentifier *)msg->data, (TestProtocol::Base *)msg->sap() );
	rtgTransitionEnd();
	enterState( 2 );
}

INLINE_CHAINS void CellHandler_Actor::chain13_testAlarmArmed( void )
{
	// transition ':TOP:Idle:J4D7FC7AF003F:testAlarmArmed'
	rtgChainBegin( 2, "testAlarmArmed" );
	exitState( rtg_parent_state );
	rtgTransitionBegin();
	transition13_testAlarmArmed( (const PeripheralIdentifier *)msg->data, (TestProtocol::Base *)msg->sap() );
	rtgTransitionEnd();
	enterState( 2 );
}
	...
	for( int stateIndex = getCurrentState(); ; stateIndex = rtg_parent_state[ stateIndex - 1 ] )
		...
		case 2:
			...
			case 5:
				...
				switch( signalIndex )
				{
				case TestProtocol::Conjugate::rti_disarm:
					chain7_Disarm();
					return;
				case TestProtocol::Conjugate::rti_arm:
					chain6_Arm();
					return;
				case TestProtocol::Conjugate::rti_alarm:
					chain10_SoundAlarms();
					return;
				case TestProtocol::Conjugate::rti_selfTest:
					chain4_LaunchSelfTest();
					return;
				default:
					break;
			...
			case 8:
				...
				case TestProtocol::Base::rti_breakin:
					chain12_testAlarmActive();
					return;
				case TestProtocol::Base::rti_disarmed:
					chain14_testAlarmDisarmed();
					return;
				case TestProtocol::Base::rti_armed:
					chain13_testAlarmArmed();
					return;
				default:
					break;
				}
